...
abstract class Util
{
    ...
    public static function flattenParams(
        ...
        foreach ($params as $key => $value) {
            ...
            // Skip RequestOptions objects - they shouldn't be in params
            if ($value instanceof \Stripe\Util\RequestOptions) {
                continue;
            }
            
            // Handle ApiResource objects by converting to their ID
            if ($value instanceof \Stripe\ApiResource) {
                $value = $value->id;
            }
            ...
            if (self::isList($value)) {
                ...
                // Skip other objects that can't be converted to string
                continue;
            } else {
    ...
    public static function flattenParamsList(
        ...
        foreach ($value as $i => $elem) {
            // Skip RequestOptions objects - they shouldn't be in params
            if ($elem instanceof \Stripe\Util\RequestOptions) {
                continue;
            }
            
            // Handle ApiResource objects by converting to their ID
            if ($elem instanceof \Stripe\ApiResource) {
                $elem = $elem->id;
            }
    ...
    {
        // Handle objects that shouldn't be here
        if (\is_object($key)) {
            // Skip RequestOptions objects
            if ($key instanceof \Stripe\Util\RequestOptions) {
                return '';
            }
            // Convert ApiResource objects to their ID
            if ($key instanceof \Stripe\ApiResource) {
                $key = $key->id;
            } else {
                // For other objects, return empty string to avoid errors
                return '';
            }
        }
